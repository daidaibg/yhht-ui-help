...
import { DocumentEvent, DocumentAttribute } from '@/document'

console.log(Yhui, vscode);
export class YhhtCompletionItemProvider implements CompletionItemProvider<CompletionItem> {
 ...
 * @param line 当前行
 */
  matchTag(reg: RegExp, txt: string, line: number): any | string | undefined {
 ...
 */
  ...
  getAttrValues(tag: string, attr: string): string[] {
    const document: Record<string, any> = Yhui
    const attributes: any = document[tag].attributes || []
    const attribute: any = attributes.find((attribute: any) => attribute.name === attr)
    if (!attribute) {
      return []
    }
    const values = attribute.value.split(/[,/\\]/).map((item: any) => item.trim())
    return values
  }
  ...
  */
  ...
  getAttrValueSuggestion(tag: any, attr: any) {
    let suggestions: any = [];
  ...
  provideCompletionItems(document: TextDocument, position: Position, token: CancellationToken, context: CompletionContext): ProviderResult<CompletionItem[] | CompletionList<CompletionItem>> {
    ...
    // console.log("是否yh-开头", /^[Y|y]h/.test(tag.text || ''));
    if (!tag || !/^[Y|y]h/.test(tag.text || '')) {
      // 如果不是yhhtui的标签(Y|yh开头) 则返回 null 表示没有hover
      return null
 ...
 */
  ...
  isAttrValueStart(tag: any, attr: any) {
    return tag && attr;
  }
  /**
  * 获取属性的提示信息
